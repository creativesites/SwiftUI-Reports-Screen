...
public struct Line2: View {
    ...
    @Binding var touchLocation1: CGPoint
    @Binding var showIndicator: Bool
    @Binding var minDataValue: Double?
    @Binding var maxDataValue: Double?
    @State private var showFull: Bool = false
    @State var showBackground: Bool = true
    var gradient: GradientColor = GradientColor(start: Colors.GradientLowerBlue, end: Colors.GradientLowerBlue)
    var index:Int = 0
    ...
    public var body: some View {
        ZStack {
            if(self.showFull && self.showBackground){
                self.closedPath
                    .fill(LinearGradient(gradient: Gradient(colors: [.clear, .clear]), startPoint: .bottom, endPoint: .top))
            ...
            if(self.showIndicator) {
                IndicatorPoint()
                    .position(self.getClosestPointOnPath(touchLocation1: self.touchLocation1))
                    .rotationEffect(.degrees(180), anchor: .center)
                    .rotation3DEffect(.degrees(180), axis: (x: 0, y: 1, z: 0))
            }
        }
    ...
    func getClosestPointOnPath(touchLocation1: CGPoint) -> CGPoint {
        let closest = self.path.point(to: touchLocation1.x)
        return closest
    }
